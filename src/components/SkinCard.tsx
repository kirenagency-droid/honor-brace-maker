...
const SkinCard = ({ name, image, price, rarity, isNew = false }: SkinCardProps) => {
  return (
    <div className={`skin-card group relative overflow-hidden rounded-lg border-2 ${rarityBorders[rarity]} bg-gradient-to-b ${rarityColors[rarity]} cursor-pointer`}>
      {/* New Badge */}
      {isNew && (
        <div className="absolute top-2 left-2 z-10 bg-accent text-accent-foreground text-xs font-bold px-2 py-0.5 rounded">
          NOUVEAU
        </div>
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 z-10">
        <h3 className="font-fortnite text-lg text-foreground tracking-wide truncate">
          {name}
        </h3>
